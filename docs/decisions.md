...
## ADR-032 — Use one source-scoped executor with bounded complete-turn history

**Context.** Administrator preview and public messaging must not drift in grounding, source authorization, history, failure, persistence, or quota behavior. Conversation history improves follow-up interpretation but is untrusted and can consume unbounded context. Public citations must not expose the internal identifiers and excerpts useful to an administrator.

**Decision.** Execute already-reserved preview and public turns through one shared service bound to the session's immutable publication. Require the current installation provider/model snapshot, derive source IDs/top-K/similarity/instructions/fallback only from that publication, and retain grounded citation validation. Use `recent_completed_turns_v1`: admit newest complete user/assistant pairs within `RAG_CHAT_HISTORY_MAX_TOKENS` (default `1000`), return them chronologically, omit oldest pairs over budget, and exclude failed/partial/current messages. Treat history as untrusted interpretive context, never evidence. No-evidence uses the configured fallback without chat generation. Persist one public-safe citation projection; return internal IDs/similarity/excerpts only as a separate bounded administrator diagnostic projection. URL sources may expose their validated source URL; Markdown sources may expose only a validated public HTTP(S) canonical URL retained from `canonical_url` or `canonical` YAML frontmatter. When public citation display is disabled, validate the grounded provider answer first, then remove citation markers and omit the projection. Reserve/reconcile the existing installation-global provider quota; existing API-key chat retains both global and per-key scopes and its HTTP contract.

**Alternatives.** Separate preview/public orchestrators; include all history; truncate individual messages; summarize history with another model call; expose the existing diagnostic citation structure publicly; let a stale publication silently use new installation models.
