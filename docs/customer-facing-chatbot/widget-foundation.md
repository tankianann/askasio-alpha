...
Each chatbot publication selects one allowlisted widget layout. `floating` preserves the fixed left/right launcher and compact/standard panel. `inline_fullscreen` renders a full-width question field and submit button at the element named by `data-container-id`. When configured suggested questions exist, one appears beneath the field as a clickable “Try asking” prompt. It rotates every five seconds only while the field is empty and unfocused, remains static for reduced-motion visitors, and fills the field without submitting so the visitor can review or edit it. Submitting the first question moves the widget to a maximum-priority body layer and opens a modal conversation that covers the viewport while centring transcript and input content within 800 pixels. Closing it restores the widget to its original embed location.
