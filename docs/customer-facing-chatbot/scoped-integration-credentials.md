# Scoped server integration credentials
...
The administrator creates credentials under **Chatbot Integrations**, chooses one to 100 chatbot scopes, and may set an expiry. The `chatint_live_` secret contains 256 random bits, is shown once, and is stored only as a safe prefix plus SHA-256 hash. Authentication verifies the hash with `hash_equals`, status, expiry, and exact chatbot relation. Revocation is immediate; rotation means creating a replacement and revoking the old credential. Deletion cascades only scope relations.
...
The route uses the shared source-scoped execution service, session limits/idempotency/persistence, and installation-wide provider quota gate. Credential records accumulate content-free request count, last use, and provider-token usage. Dedicated atomic limits default to 120 requests per credential and 240 per IP in the shared 60-second window. API Activity remains content-free and does not misclassify the credential as a general API key.
