...
Milestone 8 exposes these browser-facing routes:
...
They resolve only an active chatbot's immutable active publication. Public message submission, session authorization after creation, deletion/restart, widget assets, integration credentials, and streaming remain unavailable.

## Origin and CORS policy

Every request requires `Origin`. The shared normalizer canonicalizes HTTP(S) scheme, case, IDN host where supported, trailing dot, IPv6 brackets, and default ports. Non-local HTTP, credentials, queries, fragments, malformed hosts, wildcard/suffix/subdomain guesses, and unlisted non-default ports are rejected. Authorization compares the normalized value with the publication's exact immutable origin list.
...
The independent ID and token each contain 256 random bits. Only the token's safe prefix and SHA-256 hash are stored. Plaintext never enters a URL, cookie, Activity record, or log. A future widget must use per-tab `sessionStorage`; this milestone ships no browser storage code.
